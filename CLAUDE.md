...
- **Sub-routers** in `src/server/routers/`:
  - `sessions.ts` — Client/session CRUD, message sending, kill/cleanup
...
- `src/components/dashboard/` — Dashboard-specific components (`DashboardLayout`, `SessionCard`, `ClientCard`, `AgentBadge`, `StatusBadge`, `ErrorDisplay`)
...
- **Singleton managers**: `getAgentManager()` returns a server-side singleton `AgentManager` — all tRPC routes share this instance.

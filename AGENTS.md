...
- **React Compiler (React 19)**: Enabled via `babel-plugin-react-compiler` — avoid manual `useMemo`/`useCallback` for render optimization and omit `useMemo`/`useCallback` dependency arrays (the compiler infers them). **`useEffect` is different**: the compiler does *not* auto-add deps to effects. An effect with no dependency array runs after every render — always provide an explicit dependency array for `useEffect`.

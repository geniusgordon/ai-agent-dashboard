...
/**
 * Regex matching Write tool-call titles targeting a .claude/plans/ markdown file.
 * Example: "Write /home/gordon/.claude/plans/harmonic-chasing-knuth.md"
 * Assumes absolute Unix paths (leading `/`). Plan file names are generated by
 * Claude Code as `<adjective>-<adjective>-<name>.md`, so \S+ is sufficient.
 */
const PLAN_FILE_WRITE_RE = /^Write\s+(\/\S+\/\.claude\/plans\/\S+\.md)$/;

/**
 * Scan events (newest-first) for the latest tool-call whose title writes
 * to a `.claude/plans/*.md` file. Returns the file path or null.
 */
export function extractPlanFilePath(
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    ...
    const match = PLAN_FILE_WRITE_RE.exec(title);
